...
export function ColumnsMenu({ prefs, onChange }: { prefs: ColumnPrefs; onChange: (next: ColumnPrefs) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full right-0 z-20 mt-1 min-w-48 rounded-lg border border-zinc-200 bg-white py-1 shadow-lg dark:border-red-500/15 dark:bg-[#171114]">
          {ALL_COLUMNS.map((key) => (
            <label
              key={key}
