...
export function useTrackerToken() {
  return useQuery({
    queryKey: ["mm2-tracker-token"],
    queryFn: async () => {
      const { token } = await pb.send<{ token: string }>("/api/mm2/tracker-token", { method: "GET" });
      return token;
    },
    staleTime: Infinity,
  });
...
export function useRegenerateTrackerToken() {
  ...
  return useMutation({
    mutationFn: async () => {
      const { token } = await pb.send<{ token: string }>("/api/mm2/tracker-token/regenerate", { method: "POST" });
      return token;
    },
    onSuccess: (token) => {
      queryClient.setQueryData(["mm2-tracker-token"], token);
    },
  });
}
