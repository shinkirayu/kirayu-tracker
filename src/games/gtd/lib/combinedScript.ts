/**
 * Embedded copy of ../../../../../roblox-executor-mcp-main/GardenTD_Headless_Combined.lua.
 * Kept in sync manually. All-in-one replacement for running Kaitun
 * (GardenTD_Headless_Linux.lua) and the Summon/Buy helper (summon_remote.lua)
 * as two separate scripts: full headless farm bot (bootstrap, tracker sync,
 * AutoJoin/hostLobby, difficulty-vote + restart watcher, Macro playback,
 * PerfMode, gtd_summon_config-driven auto-trade) plus gacha summon looping,
 * auto-buying configured shop units, and an always-on lobby permanent-shop
 * buyer that grabs whichever units are currently featured on the physical
 * display stands (e.g. Beaconbud/Dual Bellflower) up to whatever purchase
 * limit the server enforces for each one - detected from the purchase
 * remote's own return value rather than a hardcoded number, since that
 * limit varies per unit and can double for Robux spenders/Premium.
 */
		...
		if type(e) == "table" and type(e.ItemData) == "table" then
			...
			if type(id) == "string" and amt > 0 then
				if not itemDict[id] then
					if not TrackerImageCache[id] and SharedItemData then
						local okItem, item = pcall(SharedItemData.GetItem, id)
						if okItem and item and item.Params then
							local img = trackerExtractImage(item.Params)
							if img then
								TrackerImageCache[id] = img
							end
						end
					end
					itemDict[id] = {
...
task.spawn(function()
	local RETRY_INTERVAL = 0.35
	local RESTART_SETTLE_DELAY = 0.5
	...
	while true do
		...
		local ok, err = pcall(function()
			...
			if outcomeVisible then
				...
				if tick() - outcomeVisibleSince > 10 and tick() - lastStuckWarn > 10 then
...
-- ===== Summon / Buy / Shop subsystem (ported from summon_remote.lua) =====
-- Shares the gtd_summon_config poll below with the trade subsystem instead
-- of running its own separate 5s fetch loop - both read disjoint fields off
-- the same record.
...
local ClientLoaderForShop = ModulesFolder and ModulesFolder.Parent
local SharedConfigForShop = ClientLoaderForShop and ClientLoaderForShop:FindFirstChild("SharedConfig")
local ItemDataForShop = SharedConfigForShop and SharedConfigForShop:FindFirstChild("ItemData")
local UnitsFolderForShop = ItemDataForShop and ItemDataForShop:FindFirstChild("Units")

local BoxData = {}
local ShopUnits = {} -- { id, name, price } for the manual buy_units list (shop + wandering trader)
local WTUnitTraderKey = {} -- unit_id -> "normal"/"luckyblock" for units sourced from the trader

local function ScrapeBoxData()
	local boxesFolder = UnitsFolderForShop and UnitsFolderForShop:FindFirstChild("Boxes")
	if not boxesFolder then
		return
	end
	for _, module in ipairs(boxesFolder:GetChildren()) do
...
-- Determines which units are purchasable with Seeds, for the manually
-- configured buy_units list. Two live sources, both scraped fresh each call:
--   1. LimitedUnitDisplays - the physical lobby stands showing whichever
--      units are currently featured.
--   2. WanderingTrader rotation (weekly, seeds + luckyblock traders).
local function ScrapeShopUnits()
	local found = {}
	local seen = {}
	local function addUnit(id, price, name)
		if seen[id] then
			return
		end
		seen[id] = true
		table.insert(found, { id = id, name = name or TrackerNameCache[id] or id, price = type(price) == "number" and price or 0 })
	end

	-- Limited-display units (e.g. this rotation's Beaconbud/Dual Bellflower)
	-- aren't always present in Units.Configs, so ScrapeUnitNames() can't
	-- always resolve their name - pass item.Params.Name through directly
	-- since it's already in hand here, instead of falling back to the id.
	local displays = Workspace:FindFirstChild("Map") and Workspace.Map:FindFirstChild("LimitedUnitDisplays")
	if displays and SharedItemData then
		for _, slot in ipairs(displays:GetChildren()) do
			...
			if unitId then
				...
				if ok and item and item.Params then
					addUnit(unitId, item.Params.SeedsPrice or item.Params.Price or 0, item.Params.Name)
				end
			end
		end
	end

	local WTModule = ModulesFolder and ModulesFolder:FindFirstChild("SharedWanderingTrader")
	if WTModule then
		local ok, WT = pcall(require, WTModule)
		if ok and WT then
			for _, traderKey in ipairs({ "normal", "luckyblock" }) do
				...
				if ok2 and offer and offer.Items then
					for _, item in ipairs(offer.Items) do
						if item.ItemId then
							local price = item.SeedsPrice or item.CandyCornsPrice or item.ChristmasGiftsPrice or 0
...
local function PushShopUnits()
	if not trackerRequestFunc or #ShopUnits == 0 then
		return
	end
	for _, unit in ipairs(ShopUnits) do
		pcall(function()
			local filter = HttpService:UrlEncode("(unit_id='" .. unit.id .. "')")
			local data = trackerGet("/api/collections/gtd_shop_units/records?filter=" .. filter)
			local body = { unit_id = unit.id, name = unit.name, price = unit.price }
			if data and data.items and data.items[1] then
				trackerPatch("/api/collections/gtd_shop_units/records/" .. data.items[1].id, body)
			else
				trackerPost("/api/collections/gtd_shop_units/records", body)
			end
		end)
	end
end

-- Synchronous (with periodic yields) so TrackerNameCache is actually
-- populated before ScrapeShopUnits() reads it below - the tracker's own
-- background scrape (task.spawn, gated on CONFIG.Tracker) can't be relied on
-- to have finished yet at this point in startup.
local function ScrapeUnitNames()
	if not UnitsFolderForShop then
		return
	end
	local configsFolder = UnitsFolderForShop:FindFirstChild("Configs")
	if not configsFolder then
		return
	end
	local children = configsFolder:GetChildren()
	for i, module in ipairs(children) do
		if module:IsA("ModuleScript") then
			local ok, data = pcall(require, module)
			if ok and data and data.Name then
				TrackerNameCache[module.Name] = data.Name
			end
		end
		if i % 20 == 0 then
			task.wait()
...
ScrapeUnitNames()
ScrapeBoxData()
-- Re-scraped on a loop (not just at startup) so the website's buy-unit list
-- stays current when the lobby's featured units rotate mid-session, with no
-- script redeploy needed - same 30s cadence as the lobby-shop buyer below.
task.spawn(function()
	while true do
		ScrapeShopUnits()
		pcall(PushShopUnits)
...
local function RefreshBanOnServer(boxId, unitId, shouldBeBanned)
	local remote = ReplicatedStorage:FindFirstChild("RemoteEvents") and ReplicatedStorage.RemoteEvents:FindFirstChild("BanFromUnitBox")
	if not remote then
		return
	end
...
local RF_BuyUnitBox = RemoteFunctions:WaitForChild("BuyUnitBox")
local RF_BuyUnitWithSeeds = RemoteFunctions:WaitForChild("BuyUnitWithSeeds")
local RF_WTBuyItem = RemoteFunctions:WaitForChild("WTBuyItem")

local RunningSummon, RunningBuy = false, false
...
local function StartSummonLoop()
	...
	if not data then
		warn("[Kirayu Headless] Summon box not found: " .. tostring(SummonConfig.SummonBox))
		return
	end

	if #SummonConfig.BannedUnits > 0 then
		for _, id in ipairs(SummonConfig.BannedUnits) do
			RefreshBanOnServer(data.id, id, true)
		end
	end

	pushHudLog("Summoning (" .. SummonConfig.SummonBox .. ")")
	RunningSummon = true
	local boxId = data.id
	local amount = SummonConfig.SummonAmount
	local thresh = SummonConfig.StopSummonAt

	task.spawn(function()
		while RunningSummon do
			if thresh > 0 and getSeeds() <= thresh then
				RunningSummon = false
				pushHudLog("Summon stopped (seeds threshold reached)")
				break
			end
...
local function StopSummonLoop()
	if RunningSummon then
		RunningSummon = false
		pushHudLog("Summon stopped")
	end
end

-- Robust purchase-limit detection: BuyUnitWithSeeds/WTBuyItem return true
-- only on an actual successful purchase and nil/false for EVERY rejection
...
local function StartBuyLoop()
	...
	RunningBuy = true
	pushHudLog("Buying configured units")
	...
	task.spawn(function()
		while RunningBuy do
			for _, id in ipairs(SummonConfig.BuyUnits) do
				...
				if not MaxedUnits[id] then
					local traderKey = WTUnitTraderKey[id]
					local ok, success
					if traderKey then
						ok, success = pcall(RF_WTBuyItem.InvokeServer, RF_WTBuyItem, traderKey, id)
					else
			...
			if allMaxed then
				RunningBuy = false
				pushHudLog("Buy loop finished (all units maxed)")
				break
			end
			task.wait(0.5)
		end
...
-- Always on, independent of the dashboard's buy_units list - grabs whichever
-- units are currently featured on the physical lobby display stands, up to
-- whatever purchase limit the server actually enforces for each one. Uses
-- its own scrape (LimitedUnitDisplays only, no WanderingTrader) so it never
-- mixes up RF_BuyUnitWithSeeds vs RF_WTBuyItem routing with ScrapeShopUnits.
local function scrapeLimitedShopUnits()
	local found = {}
	local displays = Workspace:FindFirstChild("Map") and Workspace.Map:FindFirstChild("LimitedUnitDisplays")
	if not (displays and SharedItemData) then
		return found
	end
	for _, slot in ipairs(displays:GetChildren()) do
		local sale = slot:FindFirstChild("Model") and slot.Model:FindFirstChild("Sale")
		...
		if unitId then
			...
			if ok and item and item.Params then
				local p = item.Params
				table.insert(found, { id = unitId, name = p.Name or unitId, price = p.SeedsPrice or p.Price or 0 })
			end
		end
	end
	return found
...
local function tryBuyLimitedUnitUntilLimit(unit)
	...
	while true do
		if unit.price > 0 and getSeeds() < unit.price then
			return
		end
		...
		pushHudLog("Bought " .. unit.name .. " from lobby shop")
		task.wait(0.6)
	end
end
		...
		local ok, err = pcall(function()
			if not isInActiveMatch() then
				for _, unit in ipairs(scrapeLimitedShopUnits()) do
					tryBuyLimitedUnitUntilLimit(unit)
				end
			end
		end)
		if not ok then
			warn("[Kirayu Headless] Lobby shop buyer error (recovered): " .. tostring(err))
		end
		task.wait(30)
	end
end)

-- ===== Rejoin command (website "Rejoin" / "Rejoin All" buttons) =====
-- The website bumps gtd_summon_config.rejoin_requested_at to a fresh
-- timestamp; whichever value we see on our very first poll is just this
-- script's starting baseline (not a command to act on), so we only act on
-- values that change *after* that baseline is captured.
local LastRejoinHandledAt = nil

local function ApplyRejoinConfig(remote)
	if not remote or remote.rejoin_requested_at == nil then
		return
	end

	if LastRejoinHandledAt == nil then
		LastRejoinHandledAt = remote.rejoin_requested_at
		return
	end

	if remote.rejoin_requested_at == LastRejoinHandledAt then
		return
	end
	LastRejoinHandledAt = remote.rejoin_requested_at

	if isInActiveMatch() then
		local backToLobby = ReplicatedStorage:FindFirstChild("RemoteEvents")
			and ReplicatedStorage.RemoteEvents:FindFirstChild("BackToMainLobby")
		if backToLobby then
			pushHudLog("Rejoin requested - leaving match...")
			pcall(backToLobby.FireServer, backToLobby)
		end
	else
		pushHudLog("Rejoin requested - already in lobby, AutoJoin will join shortly")
	end
end

-- ===== Unified gtd_summon_config poll (trade + summon + buy + rejoin) =====
task.spawn(function()
	local ok, remote = pcall(FetchTradeConfig)
	if ok then
		pcall(ApplyTradeConfig, remote)
		pcall(ApplySummonConfig, remote)
		pcall(ApplyRejoinConfig, remote)
	end
	while true do
		task.wait(1.5)
		...
		if ok2 then
			...
			pcall(ApplySummonConfig, remote2)
			pcall(ApplyRejoinConfig, remote2)
		end
	end
end)
...
export function buildCombinedScript(): string {
  return SCRIPT;
}